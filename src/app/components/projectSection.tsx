...
export default function ProjectSection() {
  return (
    <div className="col-start-2 px-20">
      ...
      <div className="flex flex-row w-full pb-10 justify-center gap-5 mt-5">
        {projects.map((project, i) => {
          return (
            <div
              key={i}
              className="h-100 p-5 border border-gray-200 rounded-lg cursor-pointer hover:shadow-lg transition-shadow duration-200"
              onClick={() => {}}
            >
              <img
                src={`${project.imgSrc}`}
                className="w-full h-2/3 rounded-lg border-1 border-gray-100 object-cover"
              />
              <div className="h-1/3 p-3 mt-2">
                ...
                <div className="mt-2">
                  {project.categories.map((category, idx) => (
                    <span
                      key={idx}
                      className="text-xs bg-gray-200 text-gray-800 rounded-full px-3 py-1 mr-2"
                    >
                      {category}
                    </span>
                  ))}
